...
def select_crop():
    ...
    return render_template('sdi_search_location.html', soil_data=soil_data, messages=messages, crops=crops, crop_data=data, biochar_results=biochar_results)

# Function to get latitude and longitude of the user's zip code
def get_lat_lon_from_zip(zip_code):
    geolocator = Nominatim(user_agent="sdi_biochar_search_location2.py (rachel.baschieri@usda.gov)")
...
def calculate_distances(user_lat_lon):
    ...
    for biochar in biochar_locations:
        ...
        print(f"Biochar Sample: {biochar.Sample}, Address: {biochar.Address}")
        distances.append((biochar, distance))
    
    # Sort biochar locations by distance (ascending)
    distances.sort(key=lambda x: x[1])
    return distances

@app.route('/find_biochar', methods=['GET', 'POST'])
def find_biochar():
    messages = {}
    ...
    if request.method == 'POST':
        ...
        print(f"User latitude and longitude: {user_lat_lon}")
        
        if user_lat_lon:
            # Calculate distances and find the closest biochars
            ...
            print(f"Top 5 closest biochars: {closest_biochars}") 
        else:
            messages['error'] = "Unable to find location for the given zip code."
            print("Error")
    return render_template('sdi_search_location2.html', biochars=closest_biochars, messages=messages, soil_data={}, crop_data={})
