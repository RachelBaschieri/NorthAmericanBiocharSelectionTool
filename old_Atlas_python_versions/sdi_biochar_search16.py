    ...
    # Gather soil data
        ...
        if selected_biochar:
            selected_biochar_details = db.session.query(
                ...
                HCratio.Corglb_1ton
            ).join(ExtractableP, PlantAvailableN.ID == ExtractableP.ID).join(ExtractableNutrients, ExtractableP.ID == ExtractableNutrients.ID).join(CaCO3Eq, ExtractableNutrients.ID == CaCO3Eq.ID).join(HCratio, CaCO3Eq.ID == HCratio.ID).filter(PlantAvailableN.Sample == selected_biochar).first()

            if selected_biochar_details:
                availableN, extractableP, extractableK, extractableCa, extractableMg, caco3, corg = selected_biochar_details
                print(f"Available N : {availableN}, Extractable P : {extractableP}, Extractable K : {extractableK}, Extractable Ca: {extractableCa}, Extractable Mg: {extractableMg}, CaCO3: {caco3}, Corg: {corg}")
