...
def analyze_soil_and_biochar():
    ...
    return render_template('sdi_biochar_search_10.html', soil_data=soil_data, messages = messages, lime_message=lime_message, crops=states, crop_data=data, biochar_results=biochar_results, biochars=closest_biochars, soil_type = soil_type, moisture_message=moisture_message, organic_matter_message = organic_matter_message, priorities = priority_list)
...
#Priority selection
priority_table_mapping = {
    'N requirement': PlantAvailableN,
    'P requirement': ExtractableP,
    'K requirement': ExtractableNutrients,
    'Lime requirement': CaCO3Eq,
    'Increase soil organic matter': HCratio,
    'Sequester C': HCratio,
    'Increase drainage': SA,
    'Increase water retention': SA,
    'Biochar nearby': Biochar
}
selected_priorities = {}
@app.route('/select_priorities', methods=['GET', 'POST'])
def select_priorities():
    if request.method == 'POST':
        # Capture the selected values from the dropdown menus
        priority1 = request.form.get('priority1')
        priority2 = request.form.get('priority2')
        priority3 = request.form.get('priority3')

        # Store them in a dictionary or as needed
        selected_priorities = {
            'priority1': priority1,
            'priority2': priority2,
            'priority3': priority3
        ...
        table1 = priority_table_mapping.get(priority1)
        table2 = priority_table_mapping.get(priority2)
        table3 = priority_table_mapping.get(priority3)

        # Query the database for each priority table
        results_priority1 = db.session.query(table1).all()
        results_priority2 = db.session.query(table2).all()
        results_priority3 = db.session.query(table3).all()


        # For now, just print the selected priorities
        print(selected_priorities)


        # You can now use selected_priorities in your application logic
        return render_template('sdi_biochar_search_10.html', selected_priorities=selected_priorities, results_priority1=results_priority1, results_priority2=results_priority2, results_priority3=results_priority3)
