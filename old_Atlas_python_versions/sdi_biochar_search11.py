...
class CaCO3Eq(db.Model):
    ...
    def to_dict(self):
        return {
            'ID': self.ID,
            'Sample': self.Sample,
            'CaCO3eq_mean': self.CaCO3eq_mean,
            'CaCO3eq_STD': self.CaCO3eq_STD,
            'CaCO3lb_1ton': self.CaCO3lb_1ton
        }
...
class HCratio(db.Model):
    ...
    HC_ratio = db.Column(db.Float, key='H:Corg')
...
def calculate_distances(user_lat_lon):
    ...
    for biochar in biochar_locations:
        ...
        distances.append({**biochar.to_dict(), 'distance': round(distance,2)})
    
    distances.sort(key=lambda x: x['distance'])  # Sort biochar locations by distance (ascending)
    return distances
...
def analyze_soil_and_biochar():
    ...
    if request.method == 'POST':
        ...
        # Store soil data in the session
        session['soil data'] = soil_data
        ...
        if crop:
            ...
            #store top stuff in session
            session['top_phosphorus'] = top_phosphorus
            session['top_potassium'] = top_potassium
            session['top_nitrogen'] = top_nitrogen
            session['top_lime'] = top_lime
            ...
            if user_lat_lon:
                ...
                session['closest_biochars'] = closest_biochars
            else:
        ...
        return redirect(url_for('submit_priorities'))
    
    return render_template('sdi_biochar_search_11.html', soil_data=soil_data, messages = messages, lime_message=lime_message, crops=states, crop_data=data, biochar_results=biochar_results, biochars=closest_biochars, soil_type = soil_type, moisture_message=moisture_message, organic_matter_message = organic_matter_message)
...
@app.route('/submit_priorities', methods=['GET', 'POST'])
def submit_priorities():
    ...
    lime_message = session.get('lime_message')
    data = session.get('data')
    soil_type = session.get('soil_type')
    moisture_message = session.get('moisture_message')
    organic_matter_message = session.get('organic_matter_message')
 # Fetch all priorities from the database
    ...
    for key, p in submitted_priorities.items():
        if p == 'N requirement':
            ...
            priority_results[key] = query_result
            ...
            # Perform the query and append the result to the dictionary
            query_result = HCratio.query.order_by(desc(HCratio.Corg)).limit(5).all()
            priority_results[key] = query_result

        elif p == 'Sequester C':
            query_result = HCratio.query.order_by(desc(HCratio.HC_ratio)).limit(5).all()
            priority_results[key] = query_result

        elif p == 'Increase drainage':
            query_result = SA.query.order_by(desc(SA.Average_pore_diameter)).limit(5).all()
            priority_results[key] = query_result

        elif p == 'Increase water retention':
            query_result = SA.query.order_by(desc(SA.SA_mean_m2g)).limit(5).all()
            priority_results[key] = query_result
    ...
    return render_template('priority_results.html', soil_data=soil_data, messages = messages, lime_message=lime_message, crop_data=data, biochars=closest_biochars, soil_type = soil_type, moisture_message=moisture_message, organic_matter_message = organic_matter_message, priorities = priority_list, priority_results = priority_results)
